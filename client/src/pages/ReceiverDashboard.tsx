...
export default function ReceiverDashboard() {
  ...
  const fetchDonations = async () => {
    try {
      const baseUrl = import.meta.env.VITE_API_URL; // should point to your backend
    const [availableRes, acceptedRes] = await Promise.all([
      axios.get(`${baseUrl}/api/donations/available`),
      axios.get(`${baseUrl}/api/donations/accepted`)
      ]);
      
      const available = availableRes.data?.donations ?? [];
    const accepted = acceptedRes.data?.donations ?? [];

    setAvailableDonations(available);
    setAcceptedDonations(accepted);
    setNotifications(available.length); // now safe
    } catch (error) {
  ...
  const handleAcceptDonation = async (donationId: string) => {
    try {
      await axios.post(`${import.meta.env.VITE_REACT_APP_API_URL}/api/donations/${donationId}/accept`);
  ...
  const handleMarkComplete = async (donationId: string) => {
    try {
      await axios.post(`${import.meta.env.VITE_REACT_APP_API_URL}/api/donations/${donationId}/complete`);
      await fetchDonations(); // Refresh data
    } catch (error: any) {
